...
/**
 * Checks if Unicode support is enabled for the current environment.
 *
 * This function determines if the terminal supports Unicode characters based on the operating system and environment variables.
 * It returns `false` if Unicode is supported, otherwise it returns `true`.
 *
 * @return {boolean} - True if Unicode is not supported, false otherwise.
 *
 * @example
 * console.log(isUnicodeSupported());
 * // Output: true or false based on the current environment
 */
...
/**
 * Replaces symbols in a string using fallback replacements if necessary.
 *
 * @param {string} string - The input string containing symbols to be replaced.
 * @param {{useFallback: boolean}} [options={}] - Optional options object.
 * @param {boolean} [options.useFallback=!shouldUseMain] - Whether to use fallback symbols instead of main symbols.
 * @returns {string} - The modified string with symbols replaced.
 *
 * @throws {Error} If a fallback symbol is not found for a key in replacements.
 */
